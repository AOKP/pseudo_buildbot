...
if [ -z "$lastUpdate" ]; then
    ...
    echo "To supply a custom date use flag -d MM/dd/yyyy"
    cd $jsonParser
...
rm -q $rdir/denseChangelog_$today.txt > /dev/null;
rm -q $rdir/jsonChangelog_$today.txt > /dev/null;
...
do
    ...
    if [ -z "$log" ]; then
        ...
        # Write the changelog
        echo "¶$proj_credit|$project|" >> "$rdir"/denseChangelog_$today.txt
        echo "$log" | while read line
        do
             echo "$line" >> "$rdir"/denseChangelog_$today.txt
        done
        echo "" >> "$rdir"/denseChangelog_$today.txt
    fi
done
...
#if our compiler failes delete useless denseChangelog_$today.txt
if [ $? != 0 ]; then
    cd $rdir
    rm "$rdir"/denseChangelog_$today.txt
    exit 1
fi

outfile=$fileout
if [ -z "fileout" ]; then
    outfile="$rdir"/jsonChangelog_$today.txt
fi

# make dense changelog
java -cp .:json-org.jar ParseDenseChangelog "$rdir"/denseChangelog_$today.txt $outfile
